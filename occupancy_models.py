...
Graph-based models use an edge-index representation of the household graph.
The 11 physical room/device connection pairs are stored once as an array with
shape (2, 11), matching the article-level graph specification.
"""
...
class GraphInfo:
    ...
    def num_edges(self) -> int:
        """Return the number of stored edge-index entries."""
...
def build_room_edge_index(
    room_names: Sequence[str],
    room_graph_edges: Sequence[Tuple[str, str]] = ROOM_GRAPH_EDGES,
) -> Tuple[np.ndarray, Dict[str, int]]:
    """Build an edge-index array from the predefined room graph.

    The returned ``edge_index`` has shape ``(2, E)``. Each physical
    room/device connection pair is stored once, so the default household
    graph contains 11 edge-index entries. Self-loops are not included in this
    stored representation; the graph-attention encoder adds them internally.
    """
    ...
    for src, dst in room_graph_edges:
        if src in room_to_idx and dst in room_to_idx:
            edge_pairs.append((room_to_idx[src], room_to_idx[dst]))
    if edge_pairs:
        edge_index = np.asarray(edge_pairs, dtype=np.int64).T
    else:
        edge_index = np.empty((2, 0), dtype=np.int64)
    return edge_index, room_to_idx
...
def build_room_adjacency(
    ...
    This helper is kept for convenience and visualization. The model classes
    below use ``edge_index`` rather than this dense adjacency matrix.
    """
...
def build_graph_windows(
    ...
    Returns:
        ...
        y: shape (num_samples, num_target_rooms)
        graph_info: room names, aligned sensor keys, and edge_index array
    """

    validate_input_dataframe(df, target_rooms=target_rooms)
    room_names = get_room_names(df, include_time=False)
    edge_index, room_to_idx = build_room_edge_index(room_names, room_graph_edges)
    ...
    graph_info = GraphInfo(room_names=room_names, sensor_keys=sensor_keys, edge_index=edge_index, room_to_idx=room_to_idx)
...
class EdgeIndexGATLayer(nn.Module):
    """A small graph-attention layer driven by an edge-index tensor.

    The layer avoids a PyTorch Geometric dependency while using the same
    article-level graph representation: a stored ``edge_index`` array with
    shape ``(2, E)``. The default household graph has E=11 entries, one for
    each physical room/device connection pair. Self-loops are added internally
    ...
    def _edge_mask(self, edge_index: torch.Tensor, num_nodes: int, device: torch.device) -> torch.Tensor:
        """Convert edge_index entries to a boolean attention mask."""
        ...
        if edge_index.numel() > 0:
            ...
            src_valid = src[valid]
            dst_valid = dst[valid]
            mask[src_valid, dst_valid] = True
            if self.undirected_edges:
                mask[dst_valid, src_valid] = True
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        mask = self._edge_mask(edge_index, num_nodes, x.device).view(1, num_nodes, num_nodes, 1)
...
class EdgeIndexGATEncoder(nn.Module):
    """Two-layer graph-attention encoder applied independently at each time step."""

    def __init__(self, in_channels: int, hidden_dim: int, edge_index: np.ndarray, dropout: float = 0.3):
        super().__init__()
        edge_index_array = np.asarray(edge_index, dtype=np.int64)
        if edge_index_array.ndim != 2 or edge_index_array.shape[0] != 2:
            raise ValueError(f"edge_index must have shape (2, E), received {edge_index_array.shape}")
        self.register_buffer("edge_index", torch.as_tensor(edge_index_array, dtype=torch.long))
        self.layer1 = EdgeIndexGATLayer(in_channels, hidden_dim, heads=4, concat=True, dropout=dropout)
        self.layer2 = EdgeIndexGATLayer(hidden_dim * 4, hidden_dim, heads=1, concat=False, dropout=dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = F.elu(self.layer1(h, self.edge_index))
        h = F.dropout(h, p=self.dropout, training=self.training)
        h = F.elu(self.layer2(h, self.edge_index))
        h = F.dropout(h, p=self.dropout, training=self.training)
...
class GATTemporalCNNPredictor(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.encoder = EdgeIndexGATEncoder(in_channels, gnn_hidden, edge_index, dropout=dropout)
        pooled_t = 8
...
class GATTemporalLSTMPredictor(nn.Module):
    """Graph-attention encoder followed by an LSTM head."""
...
def create_model_factory(
    ...
    if model_name == "GNN + CNN":
        return lambda: GATTemporalCNNPredictor(
            in_channels, gnn_hidden, num_nodes, seq_len, output_dim, graph_info.edge_index
        )
    ...
    if model_name == "GNN + LSTM":
        return lambda: GATTemporalLSTMPredictor(
            in_channels, gnn_hidden, num_nodes, seq_len, output_dim, graph_info.edge_index, lstm_hidden=lstm_hidden
        )
...
def train_model(
    ...
    """Train one model with BCEWithLogitsLoss and early stopping on validation loss."""
