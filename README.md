...
The public implementation represents the same article-level room graph using an edge-index array with shape `(2, 11)`, matching the manuscript notation. Each of the 11 stored entries corresponds to one physical room/device connection pair. The custom PyTorch graph-attention layer treats these stored pairs as undirected during message passing and adds self-loops internally, so no PyTorch Geometric dependency is required. This preserves the graph topology, positional encoding, temporal graph modelling structure, and model variants described in the manuscript.
